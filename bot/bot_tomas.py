...
def cortinas(update: Update, context: CallbackContext):
    if len (context.args) == 1:
            sendRequest("cortinas/set", context.args[0], update, context)
    else:
        update.message.reply_text("La cantidad de argumentos no es la correcto 😫.")
def dimmer(update: Update, context: CallbackContext):
    if len (context.args) == 1:
            sendRequest("dimmer/set", context.args[0], update, context)
    else:
        update.message.reply_text("La cantidad de argumentos no es la correcto 😫.")
def aire(update: Update, context: CallbackContext):
    if len (context.args) == 1:
            sendRequest("onoff/set", context.args[0], update, context)
    else:
        update.message.reply_text("La cantidad de argumentos no es la correcto 😫.")
def red(update: Update, context: CallbackContext):
    if len (context.args) == 1:
            sendRequest("red/set", context.args[0], update, context)
    else:
        update.message.reply_text("La cantidad de argumentos no es la correcto 😫.")
def green(update: Update, context: CallbackContext):
    if len (context.args) == 1:
            sendRequest("green/set", context.args[0], update, context)
    else:
        update.message.reply_text("La cantidad de argumentos no es la correcto 😫.")
def blue(update: Update, context: CallbackContext):
    if len (context.args) == 1:
            sendRequest("blue/set", context.args[0], update, context)
    else:
        update.message.reply_text("La cantidad de argumentos no es la correcto 😫.")

def white(update: Update, context: CallbackContext):
    
    sendRequest("blue/set", "255", update, context)
    sendRequest("red/set", "255", update, context)
    sendRequest("green/set", "255", update, context)
def off(update: Update, context: CallbackContext):
    
    sendRequest("blue/set", "0", update, context)
    sendRequest("red/set", "0", update, context)
    sendRequest("green/set", "0", update, context)
...
def download_audio (update: Update, context: CallbackContext):
    ...
    if 'temperatura' in response:
        ...
        understand = 1
    if 'subir' in response: 
        topic = "cortinas/set"
        value = "1"
        sendRequest(topic,value, update, context)
        understand = 1
    if 'bajar' in response: 
        topic = "cortinas/set"
        value = "100"
        sendRequest(topic,value, update, context)
        understand = 1
    if 'prender' in response: 
        topic = "dimmer/set"
        value = "90"
        sendRequest(topic,value, update, context)
        understand = 1
    if 'atenuar' in response: 
        topic = "dimmer/set"
        value = "25"
        sendRequest(topic,value, update, context)
        understand = 1
    if 'off' in response: 
        ...
        sendRequest(topic,value, update, context)
        understand = 1
    if 'calor' in response: 
        topic = "onoff/set"
        value = "on"
        sendRequest(topic,value, update, context)
        understand = 1
    if 'basta' in response: 
        topic = "onoff/set"
        value = "off"
        sendRequest(topic,value, update, context)
        understand = 1
    ...
    if 'casa' in response: 
        scene = "chau"
        sendRequestScene(scene, update, context)
        understand = 1
    if 'hola' in response: 
        scene = "hola"
        sendRequestScene(scene, update, context)
...
def sendRequest(topic, value, update, context):
    ...
    base_url = "http://192.168.2.112:8099/publish" #ip de la red infiniem
    
    try:
        logger.debug("try post") 
        response = requests.post(base_url, json=payload )
        if response.status_code != 200:
            logger.debug(f"Error al conectarse con la API statusCode PUTO: {response.status_code} ") 
            update.message.reply_text(f'Hola {update.message.chat.first_name}! Foxie 🦊 tuvo un problema')           
    except Exception as e: 
        update.message.reply_text("Error de comunicacion")
        logger.error(f"Error al conectarse con la API : {e}")
    
    update.message.reply_text(f'Hola {update.message.chat.first_name}! Foxie 🦊 está ejecutando tu orden')

def sendRequestScene(scene, update, context):

    	#Now we are going to Set the threshold of logger to DEBUG 
    logger.debug("second DEBUG") 
    
    payload = {"topic": scene, "value": "null"}
    
    base_url = "http://192.168.2.112:8099/publish/scenes" #ip de la red infiniem
    ...
    update.message.reply_text(f'Estoy ejecutando la orden')
...
def main():
    ...
    dp.add_handler(CommandHandler('cortinas', cortinas))
    dp.add_handler(CommandHandler('dimmer', dimmer))
    dp.add_handler(CommandHandler('aire', aire))
    dp.add_handler(CommandHandler('red', red))
    dp.add_handler(CommandHandler('green', green))
    dp.add_handler(CommandHandler('blue', blue))
    dp.add_handler(CommandHandler('white', white))
    dp.add_handler(CommandHandler('ledoff', off))
